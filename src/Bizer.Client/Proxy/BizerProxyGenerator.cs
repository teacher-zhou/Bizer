...
public class BizerProxyGenerator : DispatchProxy
{
    private IBizerInterceptor interceptor { get; set; }
    ...
    public static object Create(Type targetType, IBizerInterceptor interceptor)
    {
        var interceptorType = interceptor.GetType();
        //声明一个返回值变量
        var variable = Expression.Variable(targetType);
        var callexp = Expression.Call(typeof(DispatchProxy), nameof(DispatchProxy.Create), new[] { targetType, typeof(BizerProxyGenerator) });
        var interceptorProperty = Expression.Property(Expression.Convert(variable, typeof(BizerProxyGenerator)), nameof(interceptor));
    ...
    /// <param name="parameters">拦截器构造函数参数值</param>
    /// <returns>代理实例</returns>
    public static TTarget Create<TTarget, TInterceptor>(params object[] parameters) where TInterceptor : IBizerInterceptor
    {
    ...
    /// <returns>代理实例</returns>
    public static TTarget Create<TTarget>(IBizerInterceptor iinterceptor)
    {
